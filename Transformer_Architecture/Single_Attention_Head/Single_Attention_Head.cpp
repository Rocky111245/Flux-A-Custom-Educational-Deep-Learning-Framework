...
private:
    int d_k_; // d_k = head projection size  --> d_model / n_heads. We must decide d_k in advance.
    int sequence_length_;
    int d_model_;
    int batch_size_;
    Tensor batched_input_; //one input tensor per attention head.Keep things simple

    Tensor W_q_;  // [d_model x d_k]
    Tensor W_k_;  // [d_model x d_k]
    Tensor W_v_;  // [d_model x d_k]

    // Optional buffers
    Tensor Q_;
    Tensor K_;
    Tensor V_;
    Tensor attention_scores_;
    Tensor attention_weights_;
    Tensor output_; //One output tensor per attention head.Keep things simple. Think of the attention head as a complex transformation function
    ...
    void Apply_Softmax_To_Attention_Scores() {
