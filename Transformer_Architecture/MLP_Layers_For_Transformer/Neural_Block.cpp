...
Neural_Block::Neural_Block(Tensor residual_stream_input, const std::initializer_list<Neural> layer_list, const std::initializer_list<Tensor_Loss_Function> loss_function_list) :
...
void Neural_Block::Construct_Block_Layers() {
...
//Step 1: Forward pass.
void Neural_Block::Forward_Pass() {
    assert(!layers_.empty() && "Block must have at least one layer in Neural_Block::Forward_Pass().");
...
void Neural_Block::BackPropagate(const Tensor& upstream_error_dL_da) {
    assert(block_size_>0 && "Block size must be greater than 0 in Neural_Block::BackPropagate().");
...
void Neural_Block::Perform_Checks() const {
    assert(!layers_.empty());
    assert(block_size_ == layers_.size() && "block_size_ out of sync in Neural_Block::Perform_Checks()");
    ...
    for (size_t i = 0; i + 1 < layers_.size(); ++i) {
        ...
        assert(a.rows()    == b.rows()    && "Sequence length mismatch between layers in Neural_Block::Perform_Checks()");
        assert(a.depth()   == b.depth()   && "Batch size mismatch between layers Neural_Block::Perform_Checks()");
        assert(a.columns() == b.columns() && "Feature mismatch (I_next must equal N_prev) Neural_Block::Perform_Checks()");
    }
}
...
void Neural_Block::Update_Block(const float learning_rate) {
...
void Neural_Block::Clear_Local_Cache() {
...
void Neural_Block::Clear_Block_Cache() {
...
Tensor Neural_Block::Get_Input_Clone() const {
    return input_tensor_;
}

const Tensor& Neural_Block::Get_Input_View() const {
    return input_tensor_;
}

Tensor& Neural_Block::Get_Input_Mutable() {
    return input_tensor_;
}

Tensor  Neural_Block::Get_Output_Clone() const {
    return output_tensor_;
}

const Tensor& Neural_Block::Get_Output_View()const {
    return output_tensor_;
}

Tensor& Neural_Block::Get_Output_Mutable() {
    return output_tensor_;
}

//Will only work if backpropagation has been called already. Read-only view for any purpose (debug etc.).
const Tensor& Neural_Block::Get_Downstream_Error() const {
    assert(downstream_error_leaving_block_.rows()!=0 && downstream_error_leaving_block_.columns()!=0 && downstream_error_leaving_block_.depth()!=0
        && "Downstream error has not been set in Neural_Block::Get_Downstream_Error(). Call backpropagation first.");
    assert(downstream_error_set_ && "Downstream error has not been set in Neural_Block::Get_Downstream_Error(). "
...
// void Neural_Block::Attach_Loss_Heads(const Tensor& prediction) {
...
//                                "neurones actually present in the outer layer. Error in Neural_Block::Attach_Loss_Heads()");
//     }
//
...
float Neural_Block::Compute_Loss(const Tensor& prediction, const Tensor& target) {
    float loss=0.0f;

    if (loss_functions_.empty()) {
        throw std::logic_error("No loss functions were attached to the block. Error in Neural_Block::Compute_Loss()");
    }
...
float Neural_Block::Train(const Tensor& input,
                          const Tensor& target,
