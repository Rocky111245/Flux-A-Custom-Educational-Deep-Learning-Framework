...
SingleAttentionHead::SingleAttentionHead(const Tensor &residual_stream_input, const int d_k, const bool masked_attention)
    ...
    output_(sequence_length_,d_k_,batch_size_), masked_attention_(masked_attention)
{
    if (d_k <= 0 || d_model_ <= 0 || sequence_length_ <= 0 || batch_size_ <= 0) {
        throw std::invalid_argument("Invalid tensor dimensions or d_k in SingleAttentionHead constructor.");
    }
    Initialize_Weights();
}
...
void SingleAttentionHead:: Initialize_Weights(){
...
void SingleAttentionHead:: Compute_Projections() {
    ...
    assert(residual_stream_input_copy_.columns() == W_q_.rows() &&
            "SingleAttentionHead::Compute_Projections -> Mismatch: input.columns != W_q.rows");

    assert(residual_stream_input_copy_.rows() == Q_.rows() &&
           "SingleAttentionHead::Compute_Projections -> Mismatch: input.rows != Q_.rows");

    assert(W_q_.columns() == Q_.columns() &&
           "SingleAttentionHead::Compute_Projections -> Mismatch: W_q.columns != Q_.columns");
...
void SingleAttentionHead::Compute_Attention_Scores() {
    ...
    assert(Q_.columns() == K_.columns() &&
               "SingleAttentionHead::Compute_Attention_Scores -> Q_.columns != K_.columns");

    assert(Q_.depth() == K_.depth() &&
           "SingleAttentionHead::Compute_Attention_Scores -> Q_.depth != K_.depth");

    assert(attention_scores_.rows() == Q_.rows() &&
           "SingleAttentionHead::Compute_Attention_Scores -> attention_scores_.rows != Q_.rows");

    assert(attention_scores_.columns() == K_.rows() &&
           "SingleAttentionHead::Compute_Attention_Scores -> attention_scores_.columns != K_.rows");

    assert(attention_scores_.depth() == Q_.depth() &&
           "SingleAttentionHead::Compute_Attention_Scores -> attention_scores_.depth != Q_.depth");
...
void SingleAttentionHead:: Apply_Causal_Mask() {
...
void SingleAttentionHead:: Apply_Softmax_To_Attention_Scores() {
    ...
    assert(attention_scores_.depth() == attention_weights_.depth() &&
              "SingleAttentionHead::Apply_Softmax -> Depth mismatch between scores and weights");

    assert(attention_scores_.rows() == attention_weights_.rows() &&
           "SingleAttentionHead::Apply_Softmax -> Row mismatch between scores and weights");

    assert(attention_scores_.columns() == attention_weights_.columns() &&
           "SingleAttentionHead::Apply_Softmax -> Column mismatch between scores and weights");
    ...
    for (int batch_number = 0; batch_number < batch_size_; ++batch_number) {
        for (int query_pos = 0; query_pos < sequence_length_; ++query_pos) {
            ...
            assert(exp_sum > 0.0f && "SingleAttentionHead::Apply_Softmax -> Exponential sum is zero or negative");
...
void SingleAttentionHead:: Compute_Attention_Output() {
    ...
    assert(attention_weights_.columns() == V_.rows() &&
         "SingleAttentionHead::Compute_Attention_Output -> attention_weights_.columns != V_.rows");

    assert(attention_weights_.depth() == V_.depth() &&
           "SingleAttentionHead::Compute_Attention_Output -> Depth mismatch");
    Tensor_Multiply_Tensor(output_, attention_weights_, V_);
}
void SingleAttentionHead:: Forward_Pass() {
...
// This gives a copy
Tensor SingleAttentionHead::Get_Output_Clone() const {
    return output_;
}
//This gives read-only access
const Tensor& SingleAttentionHead::Get_Output_View() const {
    return output_;
}

//This allows mutability on return value
Tensor& SingleAttentionHead::Get_Output_Mutable()  {
    return output_;
}
