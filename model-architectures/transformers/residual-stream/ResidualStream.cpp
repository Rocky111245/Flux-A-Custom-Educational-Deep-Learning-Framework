...
ResidualStream::ResidualStream(Tensor upstream_embedded_batched_tensor)
    : residual_stream_(std::move(upstream_embedded_batched_tensor))
{
    if (residual_stream_.rows()    <= 0 ||
        ...
        throw std::invalid_argument(
            "ResidualStream: upstream tensor must have positive dimensions, got ");
        }
}

// Clone a deep copy for processing (e.g., LayerNorm → Attention path)
Tensor ResidualStream::Clone() const {
    assert(residual_stream_.rows()    > 0 &&
           residual_stream_.columns() > 0 &&
           residual_stream_.depth()   > 0 && "ResidualStream::Clone()  empty tensor");

    // Release safety (if misuse happens)
    if (residual_stream_.rows()    <= 0 ||
        residual_stream_.columns() <= 0 ||
        residual_stream_.depth()   <= 0) {
        throw std::logic_error("ResidualStream::Clone() invalid state: Invalid Dimensions for residual stream");
        }
...
void ResidualStream::Add_To_Residual(const Tensor& input) {
...
void ResidualStream::Reset(const Tensor& new_stream) {
