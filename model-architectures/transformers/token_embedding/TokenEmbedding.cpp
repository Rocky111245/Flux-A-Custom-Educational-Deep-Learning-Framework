...
//  CONSTRUCTOR
//  =============================================================================
TokenEmbedding::TokenEmbedding(const int d_vocab, const int d_model)
    : d_model_(d_model),
      d_vocab_(d_vocab),
...
std::vector<float> TokenEmbedding::Get_Token_Embedding_Vector(const int token_id) const
{
    if (token_id>=d_vocab_ ) {
        throw std::invalid_argument("Token ID out of bounds in TokenEmbedding::Get_Token_Embedding_Vector");
    }
...
// Tensor is 2D here
Tensor TokenEmbedding::Get_Sequence_Embedding_Tensor(const Tensor& final_token_tensors, const int sequence_number) const {
    // Ensure the sequence number is valid
    if (sequence_number >= final_token_tensors.rows() ) {
        throw std::invalid_argument("Sequence number out of bounds in TokenEmbedding::Get_Sequence_Embedding_Tensor");
    }
...
Tensor TokenEmbedding::Get_Batch_Embedding_Tensor(const Tensor& final_token_tensors, const int start_row_number,
    ...
    assert(
        ...
        "Wrong out tensor dimensions in TokenEmbedding::Get_Batch_Embedding_Tensor");
...
int TokenEmbedding::Get_Vocab_Size()  const { return d_vocab_; }
int TokenEmbedding::Get_Model_Dimension() const { return d_model_; }

//  =============================================================================
//  SETTER METHODS
//  =============================================================================
void TokenEmbedding::Set_Token_Embedding_Value(const int token_id,const int dimension_position,const float updated_value)
{
    if (token_id >= d_vocab_ || dimension_position >= d_model_)
        throw std::invalid_argument("token_id or dimension_position out of range in TokenEmbedding::Set_Token_Embedding_Value");

    embedding_matrix_(token_id, dimension_position, 0) = updated_value;
}

void TokenEmbedding::Set_Token_Embedding_Vector(const int token_id,const std::vector<float>& embedding_vector)
{
